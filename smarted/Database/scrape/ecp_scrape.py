...
def get_course_assessment(course_code, semester=None, year=None, delivery_mode=None):
    ...
    if semester is not None and year is not None and delivery_mode is not None:
        ...
        for course_offering in soup.tbody.find_all('tr'):
            course_mode = course_offering.find(
                "td", {"class": "course-offering-mode"}).text
            course_time = course_offering.find(
                "a", {"class": "course-offering-year"}).text
            ...
            if (course_time == f"Semester {semester}, {year}") and (course_mode == delivery_mode):
                ecp_url = course_offering.find(
                    "td", {"class": "course-offering-profile"}).find("a").get('href')
                found = True
    ...
    request = urllib.request.Request(
        ecp_url, headers={"User-agent": "Mozilla/5.0"})
    data = urllib.request.urlopen(request).read()
    soup = bs.BeautifulSoup(data, 'lxml')

    ecp_links = soup.find("ul", {"class": "toc-node"}).find_all("li")
    assessment_url = [i.a.get('href')
                      for i in ecp_links if ". Assessment" in i.text][0]
    # old backup method below
    # assessment_url = soup.find("ul", {"class": "toc-node"}).find_all("li")[4].find("a").get('href')

    request = urllib.request.Request(assessment_url, headers={
...
if __name__ == "__main__":
    ...
    mode = input(
        "enter a delivery mode ('Internal', 'External' or 'Flexible Delivery'): ")
    print("loading data...\n")
