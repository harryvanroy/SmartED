...
const questions = [
  ...
  },
  {
    question_num: 2,
    text:
      "A website has a tutorial showing how to make a special graph or chart. I would learn most from:",
    a: "seeing diagrams.",
...
export default function Vark(props) {
  ...
  const handleChange = (event, key) => {
    setState({
      ...state,
      [event.target.name + String(key)]: event.target.checked,
    });
  };
  ...
  const saveResults = () => {
    for (var i = 0; i < 16; i++) {
      for (var j = 0; j < 4; j++) {
        if (
          state["checked" + String.fromCharCode(j + 65) + String(i)] === true
        ) {
    ...
    if (total_answers > 0) {
      props.setParentVarkScore({
        V:
          Math.round((scores["V"] / total_answers + Number.EPSILON) * 100) /
          100,
        A:
          Math.round((scores["A"] / total_answers + Number.EPSILON) * 100) /
          100,
        R:
          Math.round((scores["R"] / total_answers + Number.EPSILON) * 100) /
          100,
        K:
          Math.round((scores["K"] / total_answers + Number.EPSILON) * 100) /
          100,
      });
    } else {
  ...
  return (
    <Box width="100%">
      <Box>
        ...
        <Typography variant="h4">VARK quiz</Typography>
        <Typography variant="h6">
          Choose the responses you most align with (you can select multiple
